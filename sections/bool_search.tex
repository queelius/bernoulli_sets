...
We define the function $\MakeIndex \colon \Set{D} \mapsto \PS{\Set{K}}$ that maps documents to search indexes by
\begin{equation}
...
\begin{definition}
\label{def:find}
Let $\Find \colon Q \mapsto \PS{\Set{D}}$ be the unique Boolean algebra homomorphism determined by the following clauses:
\begin{enumerate}
...
This is an appropriate abstract data type for settings such as \emph{Encrypted Search}, where conventional search indexes reveal too much information to untrusted third parties.
...
Note that $\Find^\sigma$ is a deterministic algorithm that always generates the same output for the same input; it is a function rather than a distribution, but it is compatible with the Bernoulli set model described in \cref{sec:adt}.

\subsection{Encrypted search composition}
\label{sec:encrypted_search}
A simple implementation of encrypted search may be based on a substitution cipher from keys to \emph{trapdoors} using a one-way cryptographic hash function $\operatorname{h} \colon \BitSet^* \mapsto \BitSet^k$.
Suppose we have a function $\operatorname{H} \colon [q] \mapsto [q']$ that maps Boolean queries on keys to equivalent Boolean queries on trapdoors using $\operatorname{h}$.
Then the encrypted find function is given by the composition
\begin{equation}
\label{eq:encrypted_find}
\EncryptedFind = \Find^\sigma \circ \operatorname{H} \colon [q] \mapsto \PS{\Set{D}}\,.
\end{equation}
...
\begin{example}
Suppose the search indexes are positive approximate sets each with false positive rate $\fprate$.
A common type of Boolean query is the \emph{conjunction} of $k$ atomic keys $x_1, \ldots, x_k$.
The result set
\begin{equation}
    \PASet{R}[\Set{X}] = \bigcap_{j=1}^{k} \PASet{R}[x_j]
\end{equation}
is a positive approximate set with an uncertain false positive rate $[\fprate_k] = [\fprate^k, \fprate]$.
\begin{proof}
Let $\PASet{R}[x_j]$ denote the approximate result set for key $x_j$.
By \cref{cor:fprate_atomic_search}, $\PASet{R}[x_j]$ has false positive rate $\fprate$ for each $j \in \{1,\ldots,k\}$.

The proof proceeds by induction on $k$.
For the base case $k = 1$, the result follows immediately from \cref{cor:fprate_atomic_search}.

For the inductive step, suppose $\PASet{R}[x_1] \cap \cdots \cap \PASet{R}[x_{k-1}]$ is a positive approximate set with uncertain false positive rate $[\fprate^{k-1}, \fprate]$.
By the intersection rule for positive approximate sets established in \cref{sec:set_theory},
\begin{equation}
    \left(\bigcap_{j=1}^{k-1} \PASet{R}[x_j]\right) \cap \PASet{R}[x_k]
\end{equation}
is a positive approximate set with uncertain false positive rate $[\fprate^k, \fprate]$.
\end{proof}
\end{example}
