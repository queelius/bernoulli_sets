...
Suppose $T$ is a data type that overloads the \emph{member-of} predicate $\SetContains : U \times T \mapsto \{0,1\}$ and has a \emph{value constructor} $\ctor{\fprate}{\tprate}$ that is a \emph{conditional probability distribution} over values of $T$ given elements of type $2^U$.
Data type $T$ \emph{models} the abstract data type of the random approximate set over elements in $U$ with a false positive rate $\fprate$ and true positive rate $\tprate$ if \cref{asm:fprate,asm:tprate} are satisfied, i.e.,
\begin{equation}
...
%Suppose we have a value constructor $\ctor : \PS{\Set{U}} \mapsto T$ where $T$ models random approximate sets over $\PS{\Set{U}}$ with true and false positive rates $\tprate$ and $\fprate$ respectively.
...
Suppose $\ctor{\fprate}{\fprate} : 2^U \mapsto T$ (i.e., a deterministic total function) maps sets in $2^U$ to objects of type $T$ that model random approximate sets over the input.
...
Thus, we may view $\ctor{\fprate}{\tprate}$ as a function $\ctor{\fprate}{\tprate} : 2^U \mapsto 2^U$ with an \emph{image}
\begin{equation}
...
\begin{remark}
It is often trivial to implement a family of deterministic value constructors $2^U \mapsto T = \{\operatorname{f_1},\ldots,\operatorname{f_n}\}$ with distinct $\sigma$-algebras where $T$ models random approximate sets over $2^U$.
Additionally, assuming each time an approximate set is constructed, a ``random'' value constructor from $2^U \mapsto T$ is invoked, then repeated invocations on some set $A \in 2^U$ generates a frequency distribution of sets that converges to $\tilde{A}$ as $n \to \infty$, e.g., ``randomly'' seeding a Bloom filter's hash function.
\end{remark}

How do we reconcile a deterministic value constructor $\ctor{\fprate}{\tprate} : 2^U \mapsto T$ with the \emph{probabilistic model}?
...
Suppose $U$ is finite. The set of deterministic value constructors $2^U \mapsto 2^U$ has a cardinality $(2^u)^(2^u)$, and in a sense they are all compatible with the random approximate set model.
...
\begin{equation}
    s^2_\fprate = \frac{1}{|U_p|}
        \sum_{X \in U_p} \fprateob(X),
\end{equation}
is an unbiased estimator of $\Var{\alpha_n} = \fprate(1-\fprate)/\n$.
\begin{proof}
...
For the general case with $\tprate \leq 1$, each positive element is independently retained with probability $\tprate$, so on average $\p \tprate$ elements are effectively encoded.
Each encoded element still requires $-\log_2 \fprate$ bits to maintain the false positive guarantee, giving a total of $-\p \tprate \log_2 \fprate$ bits, or $-\tprate \log_2 \fprate$ bits per element.
\end{proof}
...
Given an approximate set sampled from $\tilde{A}[\fprate][\fnrate]$, the method of moments estimator for $\p = |A|$ is undefined for countably infinite universes.
