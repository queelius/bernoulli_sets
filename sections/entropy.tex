...
A representation using $B$ bits can take at most $2^B$ distinct states.
...
$S \subseteq Q(r)$ for the state $r$ encoding $S$.
Among the $n - \p$ negatives, each belongs to $Q(r)$ independently with
probability at most $\fprate$, so
$\Expect{|Q(r)|} \leq \p + \fprate(n - \p)$.
