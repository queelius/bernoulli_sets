...
\begin{equation}
\begin{split}
\lim_{n \to \infty} \Pr\!\left[(\fprate -\epsilon) n \leq \FP_n \leq (\fprate + \epsilon)n\right] =\\
...
\label{app:samp}
Suppose we have a function $f : \PS{\Set{X}[1]} \times \cdots \times \PS{\Set{X}[q]} \mapsto \Set{Y}$, and we are interested in evaluating the loss when we replace one or more of the objective input sets with particular corresponding random approximate sets.
...
\begin{itemize}
  ...
  \item $\tilde{A}[+][\fnrate]$ --- A random approximate set of $\Set{A}$ with unspecified false positive rate and false negative rate $\fnrate$
...
\begin{itemize}
  \item $\alpha_1, \alpha_2, \alpha_3$ --- Proportions of a partition of the relevant objective set used in union/intersection error rate derivations
...
\begin{itemize}
  ...
  \item $\operatorname{h} \colon \BitSet^* \mapsto \BitSet^k$ --- Cryptographic hash function mapping keys to trapdoors
\end{itemize}
