%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Distributions of binary classification measures
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
For example, if we have a universal set $\Set{X}$ and with probability $P(x)$ an element $x \in \Set{X}$ is tested for membership in a collection of sets over $\Set{X}$, then to reduce the expected error rate on membership tests elements with higher probability of begin tested should be assigned smaller error rates.
...
\begin{theorem}
\label{thm:fpbinom}
The random number of false positives $\FP$ given $\RV{N} = n$ in the first-order random approximate set $\tilde{\RV{R}}[\fprate]$ is given by
\begin{equation}
	\FP_n = n \alpha
\end{equation}
...
\begin{equation}
    E\{\frac{\FP_n}{n}\} = \frac{1}{n}E\{\FP_n\} = \fprate
\end{equation}
and its variance is
\begin{equation}
...
\begin{equation}
	f_{\alpha_n}(\fprateob_n | \fprate) = f_{\FP_n}(n \fprateob).
\end{equation}
\end{proof}
...
\begin{proof}
    ...
    Therefore, by the central limit theorem~\cite{feller}, $\alpha_n$ converges in distribution to a normal distribution with a mean $\fprate$  and a variance $\fprate(1-\fprate)/n$.
    The proof for the true positive rate follows the same logic.
\end{proof}
By \cref{eq:approxfpr,eq:approxtpr},
\begin{equation}
	\mathrm{TNR}_n \xrightarrow{d} \normdist\!\left(1-\fprate, \fprate(1-\fprate) / n\right) \text{ and } \beta_n \xrightarrow{d} \normdist\!\left(1-\tprate, \tprate(1-\tprate) / p\right).	
\end{equation}

The Bernoulli set model is the \emph{maximum entropy} distribution over $\{0,1\}^u$ subject to the constraint that each element has a fixed marginal error probability: the binomial distribution maximizes entropy over $\{0,\ldots,n\}$ for a given mean~\cite{feller}.
As a consequence, any $\alpha$-confidence intervals derived from this model are the widest possible for the indicated $\alpha$ and therefore represent a worst-case uncertainty.
...
\begin{theorem}
    ...
    \end{equation}
    where $\Phi^{-1} : [0,1] \mapsto \RealSet$ is the inverse cumulative distribution function of the standard normal.
\end{theorem}
...
Suppose we have some other function $g : \PS{\Set{X}} \mapsto \Set{Y}$ that is not a \emph{constant}, then the composition $\Fun{g} \circ \APFun{id}^\fprate_\tprate$ is some probability distribution over the codomain $\Set{Y}$.
That is, $\left(g \circ \APFun{id}^\fprate_\tprate\right)(\Set{A})$ is a \emph{random variable}.
\begin{example}
	Let $f : \PS{\{0,1\}} \mapsto \{0,1\}$ be defined as
	\begin{equation}
	f(\Set{A}) =
	\begin{cases}
	...
	\end{cases}
	\end{equation}
	The composition $f \circ \APFun{id}[\mathsmaller{.25}][\mathsmaller{.75}]$ generates Bernoulli distribution random variables, e.g., $\left(f \circ \APFun{id}[\mathsmaller{.25}][\mathsmaller{.75}]\right)(\{0\}) \sim \berdist(0.25)$.
\end{example}
...
\begin{equation}
\Prob{\SetIndicator{\tilde{A}^\fprate_\tprate}(x) | 
