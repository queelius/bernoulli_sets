...
proper subset $\subset : 2^U \times 2^U \mapsto \{0,1\}$ predicates respectively defined as (where $[\phi]$ denotes the Iverson bracket: $1$ if $\phi$ is true, $0$ otherwise)
\begin{align}
...
\begin{corollary}
	...
	\begin{equation}
	\left(1 - \tprate\fnrate\right)^{2|X|}
	\left(1 - \fprate\tnrate\right)^{2(|U| - |X|)}.
	\end{equation}
\end{corollary}
\begin{proof}
Set $\fprate_1 = \fprate_2 = \fprate$ and $\tprate_1 = \tprate_2 = \tprate$
in the preceding corollary. Since $X = Y$, the exponents $|X|+|Y| = 2|X|$
and $2|U|-|X|-|Y| = 2(|U|-|X|)$.
\end{proof}
