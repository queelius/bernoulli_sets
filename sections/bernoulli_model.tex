...
\begin{equation}
\Prob{\mathbf{1}_{\tilde{A}}(x) \neq \mathbf{1}_{A}(x)}[\mathbf{1}_{\tilde{A}}(y) \neq \mathbf{1}_{A}(y)] =
	\Prob{\mathbf{1}_{\tilde{A}}(x) \neq \mathbf{1}_{A}(x)}.
\end{equation}
...
Furthermore, within each partition block $B_i$ (as formalized in \cref{def:model_order} below), the error indicators $\{\mathbf{1}_{\tilde{A}}(x) : x \in B_i\}$ are identically distributed.
\end{axiom}
...
\begin{remark}[Relation to binary channels]
\label{rem:bsc}
At the per-element level, the Bernoulli set model is a collection of independent binary channels~\cite{shannonBSC}; equivalently, each element undergoes Warner's randomized response~\cite{warner1965}. For each $x \in U$, the indicator $\mathbf{1}_{\tilde{A}}(x)$ is the output of a binary channel with crossover probability $\fprate$ (if $x \notin A$) or $\fnrate$ (if $x \in A$).
When $\fprate = \fnrate$ (the first-order model), this reduces to a binary \emph{symmetric} channel (BSC); in general, the channel is asymmetric.
The novelty of the present framework is not the per-element channel model, which is classical, but the axiomatic \emph{set-level} model: the derivation of error-count distributions, higher-order compositions via channel matrix products, and the abstract data type formulation that any conforming implementation inherits automatically.
The compositional algebra of set operations (error rates for union, intersection, complement) is developed in the companion paper~\cite{bernoulliComposition}.
\end{remark}

\begin{remark}[Axiom economy]
\label{rem:axiom_economy}
Most results in this paper---the distributions of error counts (\cref{sec:characteristics})---and in the companion paper on composition~\cite{bernoulliComposition}---the set-operation error rates, monoidal structure, and relational probabilities---follow from \cref{asm:element_indep} alone.
\Cref{asm:fpr_fnr_r_indep} is needed only when the objective set $R$ is itself uncertain, in which case it permits the factorization of the joint distribution of $\tilde{R}$ and $R$ (\cref{sec:prob_model}) and the entropy decomposition developed in a companion paper~\cite{bernoulliEntropy}.
\end{remark}

\begin{remark}[Parametric parsimony]
\label{rem:parametric_parsimony}
A general channel on $k$ output symbols with $n$ partition blocks has $n(k-1)$ free parameters: each of the $n$ distinct rows of the transition matrix has $k - 1$ free entries.
For a single membership test ($k = 2$), an $n$-th order model therefore has $n$ parameters---one response probability per block.
When $m$ \emph{independent} membership tests are considered jointly, the observed output lies in an alphabet of size $2^m$, and a general $n$-th order channel would require $n(2^m - 1)$ parameters.
\Cref{asm:element_indep} forces the joint transition matrix to factor as a Kronecker product of per-test $2 \times 2$ channels, so each partition block contributes one parameter per test rather than $2^m - 1$.
For example, the union of two second-order Bernoulli sets induces a fourth-order model on $4$ observed symbols~\cite{bernoulliComposition}; a general fourth-order channel would have $4 \times 3 = 12$ free parameters, but the Kronecker factorization reduces this to $4$ (one response rate per block per set).
This is not merely a mathematical convenience: it reflects the physical independence of membership queries in practical implementations such as Bloom filters and perfect hash filters, in which the query outcomes for distinct elements are determined by independent hash evaluations.
\end{remark}
...
\end{equation}
\begin{remark}[Notational convention for $\alpha$ and $\beta$]
\label{rem:alpha_beta_convention}
The statistic $\alpha$ is the sample \emph{false positive rate} (an error rate on negatives), while $\beta$ is the sample \emph{true positive rate} (a response rate on positives).
This asymmetry is deliberate: in the general $n$-th order model, each block has a single \emph{response rate} $\alpha_i$ (\cref{def:model_order}), and the second-order specialization sets $\alpha_1 = \beta$ and $\alpha_2 = \alpha$.
When the false negative rate is needed as a statistic, we write $\beta_p = \FN_p / p = 1 - \beta$ (conditioned on $p$ positives; see \cref{thm:fnr}), paralleling the true negative rate $\mathrm{TNR}_n = 1 - \alpha_n$.
The subscripted forms $\alpha_n$ and $\beta_p$ always denote error rates (FPR and FNR respectively), while the bare $\beta$ denotes the complementary TPR.
\end{remark}
\begin{proposition}
...
\end{proposition}
\begin{proof}
By linearity of expectation and \cref{asm:element_indep},
$\Expect{\alpha} = \frac{1}{|A^c|}\sum_{x \notin A} \Prob{\mathbf{1}_{\tilde{A}}(x) = 1} = \fprate$
and similarly $\Expect{\beta} = \frac{1}{|A|}\sum_{x \in A} \Prob{\mathbf{1}_{\tilde{A}}(x) = 1} = \tprate$.
\end{proof}
...
A random approximate set with $\fnrate = 0$ (equivalently, $\tprate = 1$) is a random \emph{positive} approximate set denoted by $\tilde{A}_+$.
It is parameterized by its false positive rate alone: $\tilde{A}_+^\fprate$.
\end{definition}
\begin{definition}
\label{def:neg_approx_set}
A random approximate set with $\fprate = 0$ (equivalently, $\tnrate = 1$) is a random \emph{negative} approximate set denoted by $\tilde{A}_-$.
It is parameterized by its false negative rate alone: $\tilde{A}_-^\fnrate$.
\end{definition}
By these definitions, every realization of $\tilde{A}_+$ is a \emph{superset} of $A$ (no elements of $A$ are missed) and every realization of $\tilde{A}_-$ is a \emph{subset} of $A$ (no elements outside $A$ are included).
...
\label{thm:composition_rates}
	...
	This is the standard product of binary channel transition matrices~\cite{coverThomas}.
\end{theorem}
\begin{proof}
...
This contrasts with $\AT{\left(\SetUnion[\Set{A}][\Set{B}]\right)}[\fprate][\tprate]$, in which the false positive rate for the negative elements is uniformly distributed and likewise for the positive elements.
...
A companion paper on approximate maps~\cite{bernoulliMaps} develops this observation fully: every Bernoulli approximation is an instance of approximating a latent function $f' \colon X \to Y$ with per-input independent errors, and the Bernoulli Boolean is the atomic case from which all others are built by Kronecker products across the input domain.
\end{remark}
